...
variable "master_count" {
  description = "Количество master-нод (нечётное, для кворума etcd)"
  type        = number
  default     = 3
}
...
variable "master_resources" {
  description = "Ресурсы для master-нод (там же etcd)"
  type = object({
    cores         = number
    memory        = number
...
variable "preemptible" {
  description = "Использовать прерываемые (preemptible) ВМ для всех нод"
  type        = bool
  default     = true
}
